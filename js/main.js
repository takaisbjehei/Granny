...
function setupMobileControls() {
    ...
    if (isMobile) {
        ...
        // Movement Joystick (Semi mode snaps to touch but is anchored, preventing it from wandering off-screen)
        ...
        moveJoy.on('move', (evt, data) => {
            ...
            player.moveForward = forward > 0.2;
            player.moveBackward = forward < -0.2;
            player.moveRight = right > 0.2;
            player.moveLeft = right < -0.2;
        });
    ...
    window.addEventListener('beforeunload', () => {
        if (multiplayer && multiplayer.localName) {
            // Using beacon for reliable leave or synchronous call
            navigator.sendBeacon(`https://lsboatmullmcehgogjuc.supabase.co/rest/v1/players?id=eq.${multiplayer.localId}`, '');
            // Also try normal delete
