...
function setupMobileControls() {
    ...
    if (isMobile) {
        ...
        lookJoy.on('move', (evt, data) => {
            // Simulate mouse movement (Yaw and Pitch)
            const sens = 0.05;
            const euler = new THREE.Euler(0, 0, 0, 'YXZ');
            euler.setFromQuaternion(camera.quaternion);

            euler.y -= data.vector.x * sens;
            euler.x -= data.vector.y * sens;
        ...
        // Buttons
        document.getElementById('btn-interact').addEventListener('touchstart', () => player.tryInteract());
        document.getElementById('btn-drop').addEventListener('touchstart', () => player.dropItem());
    }
}
