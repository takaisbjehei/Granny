...
export class Multiplayer {
    ...
    async fetchExistingPlayers() {
        // Cleanup players older than 10 seconds to remove stale ghosts
        const staleTime = new Date(Date.now() - 10000).toISOString();
        await supabase.from('players').delete().lt('last_updated', staleTime);

        const { data } = await supabase.from('players').select('*').gt('last_updated', staleTime);
        if (data) {
            data.forEach(p => {
    ...
    updateRemotePlayer(data) {
        if (!this.remotePlayers[data.id]) {
            ...
            torso.position.y = 0.6; // Base of torso
            ...
            nameSprite.position.set(0, 2.5, 0); // Position above head
            playerGroup.add(nameSprite);

            this.scene.add(playerGroup);
            this.remotePlayers[data.id] = playerGroup;
        }
